...
function atualizarCards(dados: DadosDashboard): void {
    ...
    const receitaTotal: number = agendamentos.reduce((acumulador, item) => {
        return acumulador + Number(item.valor || 0);
    }, 0);

    const totalAgendamentos: number = agendamentos.reduce((acumulador) => {
        return acumulador + 1;
    }, 0);

    const elReceita = document.getElementById('card-total-total');
    if (elReceita) elReceita.innerText = formatarMoeda(receitaTotal);

    const elAgendamentos = document.getElementById('card-total-agendamentos');
    if (elAgendamentos) elAgendamentos.innerText = totalAgendamentos.toString();

    const elClientes = document.getElementById('card-total-clientes');
    if (elClientes) elClientes.innerText = dados.totalClientes.toString();

    const elServicos = document.getElementById('card-total-servicos');
    if (elServicos) elServicos.innerText = dados.totalServicos.toString();
